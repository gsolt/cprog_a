...
* V2.1	2006.05.04 	Zsolt Gergely		TALUS_DAT							 *
*										fnDPTblIndx							 *									
...
extern void com_check(void);
extern void rx(void);
extern void tx_command(void); 
				...
				/* Visszatörli a timer indito bitet */
 				...
 				/* Ha nem volt elõtte lekérdezés */
  				if ( fnReadSPData(sCP.sCPR[nI].nSPOffsetLek) == 0 )
  				{
  					...
  					if (nI==69) /*Radison hotel*/
					{
						fnSetStatus(70, NOT_TOPICAL);
					}
      ...
      switch (type)
      {
        ...
        case CB_MSG_TYPE:
			...
			/*Az IEC drivernek jelzi, hogy ervenyesek az adatok*/
  			...
  			if (site_inx==69) /*Radison hotel*/
			{
				fnSetStatus(70, TOPICAL);
			}
			...
			{
	  			fnSetStatus(70, TOPICAL);
				fnWriteSPStatus(sCP.sCPR[70].nSPOffsetCS, TOPICAL);
			...
			if (site_inx==69) /*Radison hotel*/
			{
				fnWriteSPData(sCP.sCPR[70].nSPOffsetLek, 0,   0,0,0,0);
				fnWriteSPStatus(sCP.sCPR[70].nSPOffsetLek, TOPICAL);
			...
			if (buff_len==4*2 &&  nRxBuf[0] == 17 && nRxBuf[1] == 7)
			{
 			...
 			{
 				if ( (site_inx != 69) && (site_inx != 70) && (site_inx != 6))
 				{
 					TALUS_EVENT(&sTAL[site_inx],rx_buffer);
 				}
 				if (  (site_inx == 69) || (site_inx == 70) )
 				{
 					if (nRxBuf[0] == 2048)
 					{
 		 				MOSCAD_sprintf(message,"Radisson event 1.");
		   			 	MOSCAD_error(message ); 				

	 					TALUS_EVENT(&sTAL[69],rx_buffer);
 					}
 					if (nRxBuf[0] == 2049)
 					{
 		 				MOSCAD_sprintf(message,"Radisson event 2.");
		   			 	MOSCAD_error(message ); 	
		   			 				
 						TALUS_EVENT(&sTAL[70],rx_buffer);
 					}
 				}
 			...
 			else if (nType == TYP_TAL && nRxBuf[0] != 2048 && buff_len == 42 * 2 )
 			{
 				...
 				if (  (site_inx == 69) || (site_inx == 70) )
 				{
					...
					if (nRxBuf[30] == 1)
 					{
		 				MOSCAD_sprintf(message,"Radisson data 1.");
		   			 	MOSCAD_error(message ); 	
		   			 	
	 					TALUS_DAT(&sTAL[69],rx_buffer);
 					}
 					if (nRxBuf[30] == 2 )
 					{
 						TALUS_DAT(&sTAL[70],rx_buffer);
 						
		 				MOSCAD_sprintf(message,"Radisson data 2.");
		   			 	MOSCAD_error(message ); 	
 			...
 			else if (site_inx == 6 && buff_len < 42 * 2)
 				{
 					AUCHAN_EVENT(rx_buffer);
 					
 				}
 			else if (site_inx == 6 && buff_len == 68 * 2)
 				{
 					AUCHAN_DAT(rx_buffer); 					
 				}
   ...
   for (nI=0;nI<sCP.nRtuNum && nI<MAX_RTU;nI++)
   {
   	for (nJ=sCP.sCPR[nI].nDCStart;nJ<sCP.sCPR[nI].nDCStart+sCP.sCPR[nI].nDCNum;nJ++)
   	{
   		...
   		if (p_col_DCAct[nJ-nOffset]>0)
   		{
			...
			if ( (nI!=69) && (nI!=70) && (nI!=6) && (nI!=33) && (sTAL[nI].nLeagNum==8))
			{
				...
				nTxBuf[nJ - sCP.sCPR[nI].nDCStart] = p_col_DCAct[nJ-nOffset];
	 		  	...
	 		  	if (MOSCAD_TxFrm(nI, tx_buffer, 10*2) !=0 )
 			  	{
			...
			if ( (nI!=69) && (nI!=70) && (nI!=6) && (nI!=33) && (sTAL[nI].nLeagNum<8))
			{
   			...
   			{
   				nTxBuf[6] = 1; 
 	 		  	if (MOSCAD_TxFrm(nI, tx_buffer, 7*2) !=0 )
 			  	{
   			...
   			{
   				nTxBuf[5]=0;
 	 		  	if (MOSCAD_TxFrm(nI, tx_buffer, 8*2) !=0 )
 			  	{
   			...
   			else if (nI==6) /*Auchan*/
   			{
   			   for (nK=0;nK<48;nK++)
   				...
   				memcpy(&nTxBuf[1],&p_col_DCAct[20],96);
 	 		  	if (MOSCAD_TxFrm(nI, tx_buffer, 49*2) !=0 )
 			  	{
   	...
   	for (nJ=sCP.sCPR[nI].nAckStart;nJ<sCP.sCPR[nI].nAckStart+sCP.sCPR[nI].nAckNum;nJ++)
   	{
   		...
   		if (p_col_SCAct[nJ-nOffset]==0)
   		{
			...
			if ( (nI!=69) && (nI!=70) && (nI!=6) && (nI!=33)&& (sTAL[nI].nLeagNum<8))
			{
   			...
   			else if (nI==33) /*Mosonmagyarovar, szeleromu*/
   			{
  				   for (nK=0;nK<8;nK++)
				   {
				   	nTxBuf[nK]=0;
...
if (nCtrK1 >= nLimitRadioK1)
	{
			...
			/* Tavirat elkuldese */
 		  	...
 		  	{
 		  		/*MOSCAD_sprintf(message,"Could not send frame RADIO lekerdezes K1, index: %d",sRAD_K1.nIndx[nActIndxK1]);
		      	MOSCAD_error(message);*/
   			}
   			else
   			{
...
if (nCtrK2 >= nLimitRadioK2)
	{
			...
			/* Tavirat elkuldese */
 		  	...
 		  	{
 		  		/*MOSCAD_sprintf(message,"Could not send frame RADIO lekerdezes K2, index: %d",sRAD_K2.nIndx[nActIndxK2]);
		      	MOSCAD_error(message);*/
   			}
   			else
   			{
...
if (nCtrK3 >= nLimitRadioK3)
	{
			...
			/* Tavirat elkuldese */
 		  	...
 		  	{
 		  		/*MOSCAD_sprintf(message,"Could not send frame RADIO lekerdezes K3, index: %d",sRAD_K3.nIndx[nActIndxK3]);
		      	MOSCAD_error(message);*/
   			}
   			else
   			{
...
{		
		...
		if ((strTE[nI].nTalusAddr >= 904 && strTE[nI].nTalusAddr <= 907 && pTAL->nIEC_DP_FSZ1!=0) || ( (strTE[nI].nTalusAddr >= 936 && strTE[nI].nTalusAddr <= 939 && pTAL->nIEC_DP_FSZ1!=0)) )
		{
			...
			{			
				nIEC_Offset = strTE[nI].nTalusAddr - 936 + 4  + pTAL->nIEC_DP_FSZ1;			
			}
		
		/* DP tabla indexe, es offsete */
		fnDPTblIndx(nIEC_Offset,&nDPTblIndx,&nMOSCAD_OffsetDP);
	
        /*MOSCAD_sprintf(message,"nDPStart: %d,nDPTblIndx: %d, nMOSCAD_OffsetDP: %d, TalusAddr: %d, IEC offset: %d",nDPStart,nDPTblIndx,nMOSCAD_OffsetDP,strTE[nI].nTalusAddr,nIEC_Offset);
        MOSCAD_error(message );*/
...
/****************************************************************************/
/* AUCHAN állomás felõl érkezõ távirat feldolgozása						*/
/****************************************************************************/
void AUCHAN_DAT( unsigned char *rx_buf)
{
int		nI,nJ;				
...
/* Mérések feldolgozása ----------------------------------------------------------------------------------------*/
	...
	nNMStart = sSep[0].nIEC_NM;
		
	for (nI=0; nI<45 && nI<sSep[0].nNMNum; nI++)
	{
		fnWriteNM( nNMStart+nI,nRxBuf[20+nI]*3.2);
	} /*end for*/


/* Egybites jelzések feldolgozása ----------------------------------------------------------------------------------------*/
memcpy(&nDIBuf[0],&nRxBuf[2],4);
memcpy(&nDIBuf[2],&nRxBuf[5],2);
memcpy(&nDIBuf[3],&nRxBuf[7],2);

memcpy(&nDIBuf[0+4],&nRxBuf[2+6],4);
memcpy(&nDIBuf[2+4],&nRxBuf[5+6],2);
memcpy(&nDIBuf[3+4],&nRxBuf[7+6],2);

memcpy(&nDIBuf[0+8],&nRxBuf[2+12],4);
memcpy(&nDIBuf[2+8],&nRxBuf[5+12],2);
memcpy(&nDIBuf[3+8],&nRxBuf[7+12],2);
...
for (nJ=0; nJ<12; nJ++)
	...
	for (nI=0; nI<16; nI++)
	{
		...
		nIEC_Offset = sSep[0].nIEC_SP+nJ*16+nI;
...
/*SEPAM-Moscad kommunikacio ------------------------------------------------------------------------------------*/

					
					nData = nRxBuf[65];	
					nIEC_Offset = sSep[0].nIEC_MT_KommHiba;
					fnWriteSPData(nIEC_Offset,nData , 0,0,0,0);

					nData = nRxBuf[66];	
					nIEC_Offset = sSep[1].nIEC_MT_KommHiba;
					fnWriteSPData(nIEC_Offset,nData , 0,0,0,0);


					nData = nRxBuf[67];	
					nIEC_Offset = sSep[2].nIEC_MT_KommHiba;
					fnWriteSPData(nIEC_Offset,nData , 0,0,0,0);
...
/*ajto nyitva ------------------------------------------------------------------------------------*/
					nData = nRxBuf[1];	
					nIEC_Offset = 254;
					fnWriteSPData(nIEC_Offset,(nData >> 15) , 0,0,0,0);


/* Ketbites jelzesek**********************************************************/
	nDPStart = 	sSep[0].nIEC_DP;
...
nRxBuf = (unsigned short *)rx_buf;
	...
	nDPStart = sSep[nSepamNum].nIEC_DP;
...
for (nI=0;nI<nEventNum && nI<4; nI++)
	{
	if (	sSep[nSepamNum].nIEC_SP > 0)
		{
		/* Ha egybites jelzés jött -------------------------------------------------------------------------------*/
		if (strTE[nI].nTalusAddr >= 4096 && strTE[nI].nTalusAddr <= 4096 + 31 )
		{
		nData = strTE[nI].nValue;		
		nIEC_Offset = strTE[nI].nTalusAddr - 4096 + sSep[nSepamNum].nIEC_SP;			
		}/*end if egybites */	
			
		if (strTE[nI].nTalusAddr >= 4096+48 && strTE[nI].nTalusAddr <= 4096 + 63 )
		{
		nData = strTE[nI].nValue;		
		nIEC_Offset = strTE[nI].nTalusAddr - (4096+16) + sSep[nSepamNum].nIEC_SP;			
		}/*end if egybites */		
		
		if (strTE[nI].nTalusAddr >= 4096+80 && strTE[nI].nTalusAddr <= 4096 + 95 )
		{
		nData = strTE[nI].nValue;		
		nIEC_Offset = strTE[nI].nTalusAddr - (4096+32) + sSep[nSepamNum].nIEC_SP;			
		}/*end if egybites */		
		...
		/* Ha 2 bitbõl képzett kétbites jelzés jött -------------------------------------------------------------------*/
			...
			if  ((nTA==sSep[nSepamNum].nIEC_DP_2BIT_KINT1 || nTA==sSep[nSepamNum].nIEC_DP_2BIT_BENT1 || nTA==sSep[nSepamNum].nIEC_DP_2BIT_KINT2 || nTA==sSep[nSepamNum].nIEC_DP_2BIT_BENT2 || nTA==sSep[nSepamNum].nIEC_DP_2BIT_KINT3 || nTA==sSep[nSepamNum].nIEC_DP_2BIT_BENT3 || nTA==sSep[nSepamNum].nIEC_DP_2BIT_KINT4 || nTA==sSep[nSepamNum].nIEC_DP_2BIT_BENT4) && (nTA>0))
			{
				if (nTA==sSep[nSepamNum].nIEC_DP_2BIT_KINT1 || nTA==sSep[nSepamNum].nIEC_DP_2BIT_BENT1)
				{
					nIEC_Offset = sSep[nSepamNum].nIEC_DP_2BIT1;
				}			
			
				if (nTA==sSep[nSepamNum].nIEC_DP_2BIT_KINT2 || nTA==sSep[nSepamNum].nIEC_DP_2BIT_BENT2)
				{
					nIEC_Offset = sSep[nSepamNum].nIEC_DP_2BIT2;
				}			

				if (nTA==sSep[nSepamNum].nIEC_DP_2BIT_KINT3 || nTA==sSep[nSepamNum].nIEC_DP_2BIT_BENT3)
				{
					nIEC_Offset = sSep[nSepamNum].nIEC_DP_2BIT3;
				}			
				if (nTA==sSep[nSepamNum].nIEC_DP_2BIT_KINT4 || nTA==sSep[nSepamNum].nIEC_DP_2BIT_BENT4)
				{
					nIEC_Offset = sSep[nSepamNum].nIEC_DP_2BIT4;
				}			
			...
			if (nIEC_Offset > 0)
			{
				if (nTA==sSep[nSepamNum].nIEC_DP_2BIT_KINT1 || nTA==sSep[nSepamNum].nIEC_DP_2BIT_KINT2 || nTA==sSep[nSepamNum].nIEC_DP_2BIT_KINT3 || nTA==sSep[nSepamNum].nIEC_DP_2BIT_KINT4)
				{					
					/* Érték beírása */
					p_col_DPL[nIEC_Offset-nMOSCAD_OffsetDP] = strTE[nI].nValue;
				}			

				if (nTA==sSep[nSepamNum].nIEC_DP_2BIT_BENT1 || nTA==sSep[nSepamNum].nIEC_DP_2BIT_BENT2 || nTA==sSep[nSepamNum].nIEC_DP_2BIT_BENT3 || nTA==sSep[nSepamNum].nIEC_DP_2BIT_BENT4)
				{
					/* Érték beírása */
					p_col_DPH[nIEC_Offset-nMOSCAD_OffsetDP] = strTE[nI].nValue;
				}
...
if (nI == 6) /*Szekesfehervar Auchan*/
	{
			/* Egy bites jelzesek ------------------------------------*/
			for (nJ=60;nJ<261;nJ++)
			{
				fnWriteSPStatus( nJ, nValue);
			}
			/* Ket bites jelzesek --------------------------------*/
			for (nJ=20;nJ<30;nJ++)
			{
				fnWriteDPStatus( nJ, nValue);
			}
			/* Meresek  --------------------------------*/
			for (nJ=10;nJ<55;nJ++)
			{
				fnWriteNMStatus( nJ, nValue);
			}
...
nReteszOffset[0] = 0;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/
																													/**/
...
/* 2. TMOK: 12-05 RTU: Front end G -> Mosonmagyaróvár 739-8 PV erõmû -----------------------*/								/**/
TMOKAllasjelzesOffsetek[2] = 260; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[2][0] =1256;						/* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[2][0] = 265;			/* G front end */															/**/
...
nReteszOffset[2] = 2;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/

/* 3. TMOK: 15-02 RTU: Front end G -> Dunakiliti I, II erõmû -----------------------*/								/**/
TMOKAllasjelzesOffsetek[3] = 277; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[3][0] =1261;						/* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[3][0] = 265;			/* G front end */															/**/
...
/* 4. TMOK: 15-06 RTU: Front end G -> Dunakiliti I, II erõmû -----------------------*/								/**/
TMOKAllasjelzesOffsetek[4] = 281; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[4][0] =1262;						/* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[4][0] = 265;			/* G front end */															/**/
...
/* 6. TMOK: 30-43 RTU: H front end -> Csór 65/1, 65/2 PV erõmû - tesztelve -----------------------*/								/**/
TMOKAllasjelzesOffsetek[6] = 422; 	/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[6][0] =2;   						  /* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[6][0] = 266;			/* A redundancia */															/**/
...
nReteszOffset[6] = 6;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/

/* 7. TMOK: 12-51 RTU: A front end -> Újrónafõ, C front end J1, J4-5, J6 -----------------------*/								/**/
TMOKAllasjelzesOffsetek[7] = 280; 	/* Az állásjelzés offsete a DP adatbázisban */								/**/

TMOK_ID[7][0] =280;   						  /* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[7][0] = 240;			/* Újrónafõ */															/**/
TMOK_ID[7][1] =1250;   						  /* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[7][1] = 263;			/* C front end */															/**/
...
/* 8. TMOK: 41-48 RTU: G front end -> Kenéz 062-9 PV erõmû -----------------------*/								/**/
TMOKAllasjelzesOffsetek[8] = 338; 	/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[8][0] =1268;   						  /* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[8][0] = 265;			/* G front end */															/**/
...
/* 10. TMOK: 42-16 RTU: G front end -> Gyõrvár 03/15, 05/1 PV erõmû -----------------------*/								/**/
TMOKAllasjelzesOffsetek[10] = 424; 	/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[10][0] =1272;   						  /* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[10][0] = 265;			/* G front end */															/**/
...
nReteszOffset[10] = 9;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/

/* 11. TMOK: 91-70 RTU: G front end -> Zalaegerszeg 0779/ PV erõmû -----------------------*/								/**/
TMOKAllasjelzesOffsetek[11] = 415; 	/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[11][0] =1273;   						  /* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[11][0] = 265;			/* G front end */															/**/
...
nReteszOffset[11] = 10;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/

/* 12. TMOK: 41-19 RTU: G front end -> Kenéz 062-9  PV erõmû -----------------------*/								/**/
TMOKAllasjelzesOffsetek[12] = 326; 	/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[12][0] =1269;   						  /* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[12][0] = 265;			/* G front end */															/**/
...
nReteszOffset[12] = 11;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/

/* 13. TMOK: 85-50 RTU: G front end -> Almásfüzitõ 012  PV erõmû -----------------------*/								/**/
TMOKAllasjelzesOffsetek[13] = 509; 	/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[13][0] =1280;   						  /* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[13][0] = 265;			/* G front end */															/**/
...
nReteszOffset[13] = 12;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/

/* 14. TMOK: 81-50 RTU: G front end -> Tata Moowa  PV erõmû -----------------------*/								/**/
TMOKAllasjelzesOffsetek[14] = 504; 	/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[14][0] =1286;   						  /* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[14][0] = 265;			/* G front end */															/**/
...
nReteszOffset[14] = 13;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/

/* 15. TMOK: 41-50 RTU: G front end -> Gyöngyösfalu PV erõmû -----------------------*/								/**/
TMOKAllasjelzesOffsetek[15] = 274; 	/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[15][0] =1288;   						  /* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[15][0] = 265;			/* G front end */															/**/
...
nReteszOffset[15] = 14;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/

/* 16. TMOK: 41-66 RTU: G front end -> Gyöngyösfalu PV erõmû -----------------------*/								/**/
TMOKAllasjelzesOffsetek[16] = 314; 	/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[16][0] =1289;   						  /* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[16][0] = 265;			/* G front end */															/**/
...
nReteszOffset[16] = 15;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/

/* 17. TMOK: 92-59 RTU: G front end -> Nova 2037/2 PV erõmû -----------------------*/								/**/
TMOKAllasjelzesOffsetek[17] = 475; 	/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[17][0] =1292;   						  /* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[17][0] = 265;			/* G front end */															/**/
...
nReteszOffset[17] = 16;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/

/* 18. TMOK: 41-78 RTU: G front end -> Kõszeg 0117 PV erõmû -----------------------*/								/**/
TMOKAllasjelzesOffsetek[18] = 312; 	/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[18][0] =1297;   						  /* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[18][0] = 265;			/* G front end */															/**/
...
nReteszOffset[18] = 17;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/
